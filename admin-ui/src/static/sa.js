...
(function () {
	...
	/** 
		...
		如果你觉得此函数难以理解, 也可以不用, 可以用你比较熟悉的原生写法 
		如果你对axios更熟悉, 也可以引入axios, 步骤为:
				先在cmd中安装:	
					npm install axios -S
				然后在main.js里引入
	...
	sa.ajax = function (url, data, success200, cfg) {
		...
		var defaultCfg = {
			...
			success3000: function (res) { // 3000 都是为登录，获取唯有权限
				sa.loginState = false;
				Notification({
					title: "页面",
					message: res.message,
			...
			success10000: function (res) { // code=500, 代表失败 
				Notification({
					title: "异常",
					message: res.message?( res.message.length > 100? res.message.substr(0,100): res.message):"接口异常10000",
					type: 'error'
				})
			},

			success404: function (res) { // code=401, 代表未登录
				Notification({
					title: "页面",
					message: res.message,
					type: 'warning'
			...
			errorfn: function (error) { // ajax发生异常时的默认处理函数
				...
				Notification({
					title: "异常消息",
					message:  error.responseJSON.code + "：" + error.responseJSON.message,
					type: 'error'
				})
				//return layer.alert();
			},
		...
		return $.ajax({
			url: cfg.baseUrl + url,
			type: cfg.type,
			data: cfg.type == "GET" ? data : JSON.stringify(data),
...
(function () {
	...
	me.ok = function (msg) {
		msg = msg || '操作成功';
		Notification.success({ message: msg, duration: 2000 })
	}

	me.ok2 = function (msg) {
		msg = msg || '操作成功';
		Notification.success({ message: msg, duration: 6000 })
	}

	// 操作失败的提示  
	me.error = function (msg) {
		msg = msg || '操作失败';
		Notification.warning({ message: msg, duration: 2000 })
	}

	me.error2 = function (msg) {
		msg = msg || '操作失败';
		Notification.warning({ message: msg, duration: 5000 })
	}
	...
	if (true) {
		...
		me.forDate = function (inputTime, way) {
			...
			if (typeof way == 'string') {
				return way.replace("yyyy", y).replace("MM", m).replace("dd", d).replace("HH", h).replace(
					"mm", minute).replace("ss", second);
			}
			return y + '-' + m + '-' + d;
		};
	...
	if (true) {
		...
		me.keyListGet = function (key) {
			try {
				...
				if (str == undefined || str == null || str == '' || str == 'undefined' || typeof (JSON.parse(
					str)) == 'string') {
					//alert('key' + str);
					str = '[]';
				}
...
if (window.Vue && !Vue.prototype.sa) {
	Vue.prototype.sa = sa;
}

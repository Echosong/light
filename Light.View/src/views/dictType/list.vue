...
async function f5() {
    loading.value = true
    const {data} = await sa.put("/dictType/listPage", p.value);
    loading.value = false
...
function exportFile() {
    sa.download("/dictType/export", p.value)
}
...
function del(data) {
    sa.confirm('是否删除，此操作不可撤销', async function () {
        let res = await sa.delete("/dictType/delete/" + data.id);
        console.log(res)
...
async function updateSwitch(row) {
    await sa.post('/dictType//save', row);
    sa.ok("更新成功", true)
}

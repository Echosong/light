<template>
 ...
 <el-input v-model="p.name" placeholder="模糊查询"></el-input>
  ...
  <el-table-column  label="产品描述"  :show-overflow-tooltip="true" prop="info" ></el-table-column>
...
const params = {pageSize:10,page:1, total: 0, name:'',price:''}
...
function exportFile() {
    sa.download("/product/export", p.value)
}
...
async function updateSwitch(row) {
    await sa.post('/product//save', row);
    sa.ok("更新成功", true)
}

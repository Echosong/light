...
<el-form-item label="运营人"  v-if="!query.operation">
 <select-data v-model="p.operation" routeName="user" ></select-data>
  ...
  <el-table-column  label="方案"   prop="scheme" ></el-table-column>
  <el-table-column  label="渠道等级"   prop="grade" ></el-table-column>
  ...
  <el-table-column  label="回本日期"   prop="backTime" ></el-table-column>
...
function exportFile() {
    sa.download("/order/export", p.value)
}
...
function del(data) {
    sa.confirm('是否删除，此操作不可撤销', async function () {
        let res = await sa.delete("/order/delete/" + data.id);
        console.log(res)
